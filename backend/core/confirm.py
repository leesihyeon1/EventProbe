...
# 에러 기반이 마커 없이도 'DB 에러 유발' 로 잡히도록 하는 SQL/DBMS 에러 시그니처.
# 대상 DB 를 미리 알 수 없으므로 주요 DBMS 의 에러 문구를 폭넓게 인식한다 —
# 코퍼스는 core.error_signatures 단일 소스(analyzer 의 ERROR_LEAK_PATTERNS 와 동일 JSON).
# 예전엔 여기 별도 하드코딩 목록이 있어 analyzer 와 드리프트했다.
_SQL_ERROR_RE = _error_signatures.sql_error_regex()
