...
_VERDICT_SYSTEM = (
    ...
    "단정하지 말 것. 403 등은 payload 를 막은 것일 수도, 경로 자체가 원래 거부되는 것일 수도 있다. "
    "'요청이 403으로 거부됨(WAF 필터인지 경로 자체 제한인지 미확인)' 수준으로 서술하고, priority 에 "
    "'정상 파라미터로 baseline 비교'를 제안한다. 이때 confidence 는 80 을 넘기지 말 것.\n"
