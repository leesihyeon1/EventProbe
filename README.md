...
AI 분류·응답 분석에 전달하는 헤더는 공통 정책(`backend/core/ai_privacy.py`)으로
Cookie, Set-Cookie, Authorization, Proxy-Authorization, Host 및 등록된 API 키·인증·CSRF 토큰
헤더를 대소문자와 관계없이 제외합니다. 실제 테스트 대상에 보내는 요청과 AI 제공자 인증은 유지됩니다.
이 처리는 헤더 필터링이며, URL·쿼리·본문·판정 증거 안의 민감정보나 임의의 사용자 정의 인증 헤더까지
익명화하지는 않습니다. AI 종합 판정에도 요청 맥락과 판정 근거가 전달됩니다.

### RAG 검색

등록한 PDF·URL·텍스트는 로컬에서 청킹하고 BM25와 임베딩 검색을 결합해 관련 근거를 찾습니다.
임베딩 모델·본문·청킹 버전이 바뀐 소스는 BM25로 폴백하며 화면에 재색인 필요 상태를 표시합니다.
검색된 문서 발췌는 AI 기능 사용 시 생성·조치 프롬프트에 포함됩니다. 검색 품질은 다음 골든셋으로
오프라인 평가할 수 있습니다.
...
판정 엔진(`backend/core/`)의 순수 함수에 대한 단위 테스트가 있습니다.
**네트워크 요청을 전혀 보내지 않으므로**(가짜 응답 문자열로 판정 로직만 검증),
어떤 대상에도 접속하지 않고 오프라인/무허가로 안전하게 실행됩니다.

```bash
python -m pytest backend/tests -q
```

- `test_analyzer.py` — 위험도 산정('차단 안 됨 ≠ 취약' 오탐 방지), 증거 기반 성공 격상, WAF 지문
- `test_confirm.py` — 오라클 확증 스캔(시간/불린 기반, 마커 반사)의 확증·기각 판정
- `test_cve_matcher.py` — 로컬 CVE 지문 매칭·정렬·중복 제거
